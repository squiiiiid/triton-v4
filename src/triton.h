...
*/
		...
		void compute_dam_failure();
		
		T compute_dam_H(struct dam_arg* dam);//溃口水深
		
		T compute_dam_Uc(struct dam_arg* dam,T h);//临界起动流速
		
		T compute_dam_V(struct dam_arg* dam);//溃口出流速度
		
		T compute_dam_tau(struct dam_arg* dam,T h);//溃口底面剪应力
		
		T compute_dam_dHc(struct dam_arg* dam,T tau,T dt);//溃口展深变化		
...
//compute_dam_failure
	...
	template<typename T>
	void triton<T>::compute_dam_failure()
	{
		//T* dam_mask;//->dam_attri
		//dam_mask = (T*)malloc(cols*rows*sizeof(T));
...
exitdam:		
		...
		ax=dam0.L*cos(dam0.rot);
		ay=dam0.L*sin(dam0.rot);
		crot=dam0.rot-1.5708;
		bx=Bu*cos(crot);
		by=Bu*sin(crot);
		
		tx=dam0.cx-0.5*bx;
		ty=dam0.cy-0.5*by;
		if(((int)tx)<idx1)	idx1=tx;
		if(((int)tx)>idx2)	idx2=tx;
		if(((int)ty)<idy1)	idy1=ty;
		if(((int)ty)>idy2)	idy2=ty;
		
		tx+=bx;
		ty+=by;
		if(((int)tx)<idx1)	idx1=tx;
		else if(((int)tx)>idx2)	idx2=tx;
		if(((int)ty)<idy1)	idy1=ty;
		else if(((int)ty)>idy2)	idy2=ty;
		...
		T jx,iy,dot,a2,b2,db,dh;
		a2=ax*ax+ay*ay;
		b2=bx*bx+by*by;
		for(int i=idy1;i<=idy2;++i)
		{
			for(int j=idx1;j<=idx2;++j)
			{
				...
				iy=((T)i)+0.5-ty;
				dot=jx*ax+iy*ay;
				if(dot<0.0||dot>a2)	continue;
				dot=jx*bx+iy*by;
				if(dot<0.0||dot>b2)	continue;
				db=dot/b2-0.5;
				if(db<0.0)	db=-db;
				db*=2.0;
	...
	template<typename T>
	T triton<T>::compute_dam_Uc(struct dam_arg* dam,T h)//临界起动流速，完成 
	{
		return 1.14*sqrt((dam->gamma_s-dam->gamma)*dam->gravity*dam->D/dam->gamma)*pow(h/dam->D,1.0/6.0);
	}
	...
	template<typename T>
	T triton<T>::compute_dam_tau(struct dam_arg* dam,T h)//溃口底面剪应力，完成 
	{
		T V2,tmp;//溃口底面流速平方
		V2=host_vec[QX][dam->idx+dam->idy*cols];
		V2*=V2;
		tmp=host_vec[QY][dam->idx+dam->idy*cols];
		V2+=tmp*tmp;//点速度 
		
		return dam->gamma*dam->n2*V2/sqrt(h);
	}
	
	template<typename T>
	T triton<T>::compute_dam_dHc(struct dam_arg* dam,T tau,T dt)//溃口展深变化，完成 
	{
		return 0.0513*pow(tau-(0.05*(dam->gamma_s-dam->gamma)*dam->D),1.3)/(1.0-dam->e)*dt;
	}
